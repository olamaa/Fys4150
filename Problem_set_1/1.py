...
for i in range(len(u_all)):
    abs_error, rel_error, max_err = np.zeros(len(u_all)), np.zeros(len(u_all[i])), np.zeros(len(u_all[i]))
    for j in range(1, len(u_all[i])-1):
        #print(u[i], u_all[i][j], v_all[i][j])
        abs_error[j] = np.abs(u_all[i][j] - v_all[i][j])
        rel_error[j] = abs_error[j]/u_all[i][j]
    max_err[i] = np.amax(rel_error[1:-1])
    print(f'At n = {10**(i+1):.1e} the maximum relative error is {max_err[i]:.5e}')# and the minimum {np.amin(rel_error[1:-1])}')
    if i == 2:
        ax[0].plot(x_all[i][1:-1], np.log10(abs_error[1:-1]), label = f'n = {10**(i+1):.1e}', color=colors[i])
        ax[1].plot(x_all[i][1:-1], np.log10(rel_error[1:-1]), label = f'n = {10**(i+1):.1e}', color=colors[i])
    else:
        ax[0].plot(x_all[i][1:-1], np.log10(abs_error[1:-1]), alpha=0.3, label = f'n = {10**(i+1):.1e}', color=colors[i])
        ax[1].plot(x_all[i][1:-1], np.log10(rel_error[1:-1]), alpha=0.3, label = f'n = {10**(i+1):.1e}', color=colors[i])
    #else:
    #    plt.plot(x_all[i][1:-1], np.log10(rel_error[1:-1]), label = f'n = {10**(i+1):.1e}', alpha = 0.3, color=colors[i])
#plt.ylim(-0.025, 0.2)
plt.legend()
plt.show()
